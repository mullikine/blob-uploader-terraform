resource "aws_ecs_service" "blob_uploader_app_service" {
  ...
  task_definition = "${aws_ecs_task_definition.blob_uploader_app.family}:${max(aws_ecs_task_definition.film_ratings_app.revision, data.aws_ecs_task_definition.film_ratings_app.revision)}"
